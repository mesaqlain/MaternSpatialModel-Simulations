...
logL_binom <- function(parms_to_estimate, phi, re, det.Q, Q, T.mat, mu, eta, family, y.response){
  n <- length(eta) #No. of observations
  ys<-y.response
  s0 <- parms_to_estimate[1]
  wt <- (1/phi)*as.numeric((family$mu.eta(eta))^2/family$variance(mu))
  ...
  l <- sum(dbinom(ys, size=1, prob=mu, log=TRUE)) -
    (0.5)*t(as.vector(re))%*%(Q/s0)%*%as.vector(re) + ## This was + before
    0.5*(det.Q_new) -
